...
#include "ui_zslingcontroldialog.h"
...
std::list<std::string> ZSlingcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSlingcoinControlDialog::listMints;

ZSlingcoinControlDialog::ZSlingcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSlingcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSlingcoinControlDialog::~ZSlingcoinControlDialog()
{
    delete ui;
}

void ZSlingcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSlingcoinControlDialog::updateList()
{
...
void ZSlingcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSlingcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSlingcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSlingcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSlingcoinControlDialog::GetSelectedMints()
{
...
void ZSlingcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
