                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSLING ought to be enough for anybody." - Bill Gates, 2017
    ui->zSLINGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSLINGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSLING->setEnabled(false);
        ui->pushButtonMintzSLING->setToolTip(tr("zSLING is currently disabled due to maintenance."));

        ui->pushButtonSpendzSLING->setEnabled(false);
        ui->pushButtonSpendzSLING->setToolTip(tr("zSLING is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSLINGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSLING_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSLING is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSLING...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSLING in ") +
...
void PrivacyDialog::on_pushButtonSpendzSLING_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSLING is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSLING
        sendzSLING();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSLING
    sendzSLING();
}

void PrivacyDialog::on_pushButtonZSlingcoinControl_clicked()
{
    ZSlingcoinControlDialog* zSLINGControl = new ZSlingcoinControlDialog(this);
    zSLINGControl->setModel(walletModel);
    zSLINGControl->exec();
}

void PrivacyDialog::setZSlingcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSLINGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSLING()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Slingcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSLINGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSLING is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SLING </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSLING</b>";
    ...
    // use mints from zSLING selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSlingcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSlingcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsling selector in case it was used
    ZSlingcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSLING Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Slingcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSLING Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSLING </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSLING "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSLING "));
    ui->labelzSLINGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSLINGSyncStatus->setVisible(fShow);
}
